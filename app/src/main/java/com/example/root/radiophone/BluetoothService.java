...
public class BluetoothService extends Service {
    ...
    public static final UUID NOTIF_UUID = UUID.fromString("6e400003-b5a3-f393-e0a9-e50e24dcca9e");
    ...
    private BluetoothGattCharacteristic wr;
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            wr = gatt.getService(SERVICE_UUID).getCharacteristic(WRITE_UUID);
            nf = gatt.getService(SERVICE_UUID).getCharacteristic(NOTIF_UUID);


            setCharacteristicNotification(nf, true);
        ...
        final Intent intent = new Intent(action);
{
            ...
            if (data != null && data.length > 0) {
                ...
                for(byte byteChar : data)
                   ...
                   // System.out.println(String.valueOf(String.format("%02X", k)));


                System.out.println(hexToString(stringBuilder.toString()));

                Log.d("BLUETOOTHRECEIVE", stringBuilder.toString());

                intent.putExtra(EXTRA_DATA, new String(data));


                //intent.putExtra(EXTRA_DATA, convertHexToString(data));
            }
        }
        sendBroadcast(intent);
    }
    ...
    public void send(byte[] data) {
        ...
        if (wr == null || data == null || data.length == 0) {
            // Do nothing if there is no connection or message to send.
            return;
        }
        // Update TX characteristic value.  Note the setValue overload that takes a byte array must be used.
        wr.setValue(data);
        mBluetoothGatt.writeCharacteristic(wr);
    }
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId){
        ...
        if (intent.getExtras().containsKey("message")) {
                command = intent.getExtras().getString("message");
                Log.d("command extra", command);
                        send(command);
        }
